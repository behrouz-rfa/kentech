...
func ToMap(from interface{}, method ...Method) InputMap {
	if len(method) > 1 {
		logger.General.Component("Mongo Utils").Panic("ToMap only accepts one method")
	}
	...
	case primitive.M, primitive.D, primitive.E, primitive.A, primitive.Regex:
		...
		if err != nil {
			logger.General.WithError(err).Panic("Could not marshal primitives toMap")
		}

		err = bson.Unmarshal(marshaled, &jsonFields)
		if err != nil {
			logger.General.WithError(err).Panic("Could not Unmarshal primitives toMap")
		}
